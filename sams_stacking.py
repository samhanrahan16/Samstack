...
class samstack:
    ...
    def simple_stack(self):
        ...
        map_names = [i for i in list(map_library.keys())]
        ...
        bin_names = list(subcatalog_library.keys())
        ...
        for iwv in range(nwv):
            ...
            stacked_stats = {}
            stacking_type = {}
            stacking_types = ['map','noise']
            for name in bin_names:
                if len(subcatalog_library[name][0]) >0:
                    ngals_per_pix = np.zeros([map_shape[0],map_shape[1]])
                    ngals_per_pix_n = np.zeros([map_shape[0],map_shape[1]])
                    ra = subcatalog_library[name][0]
                    dec = subcatalog_library[name][1]
                    ty,tx = cw.wcs_world2pix(ra,dec,0)
                    ty_n = np.random.uniform(15,int(map_shape[1]) - 15,len(ra))
                    tx_n = np.random.uniform(15,int(map_shape[0]) - 15,len(ra))
                    ...
                    real_x_n = np.round(tx_n).astype(int)
                    real_y_n = np.round(ty_n).astype(int)
                    ...
                    if nt >0:
                        ...
                        for ni in range(nt):
                            ...
                            ngals_per_pix_n[real_x_n[ni],real_y_n[ni]] += 1.0

                    ngals_per_pix_all[name] = ngals_per_pix
                    ngals_per_pix_all_n[name] = ngals_per_pix_n
                else:
                    ngals_layer[name] = 1
                    ngals_per_pix_all[name] = 'empty'
                    ngals_per_pix_all_n[name] = 'empty'
                    
            stacking_type['map'] = ngals_per_pix_all
            stacking_type['noise'] = ngals_per_pix_all_n

            for t in stacking_types:
                for name in bin_names:
                    if stacking_type[t][name] == 'empty':
                        ...
                        gals_loc_map = stacking_type[t][name]
                        ind_gals = np.where(stacking_type[t][name] >=1)
                        ...
                        if t == 'map':
                            ...
                            stacked_noises[name] = stacked_flux
    ...
    def save_stacked_fluxes(self,save_path):
        ...
        stdevs_all = []
        ngals_bin_all = []
        for map_name,wv in zip(self.map_names,self.wavelengths):
            mid_vals = []
            stacked_fluxes = []
            stacked_noises = []
            means = []
        ...
        results_table = Table([tbl_maps,wavelengths,bin_names_all,ngals_bin_all,stacked_fluxes_all,stacked_noises_all,means_all,medians_all,stdevs_all],names=['map','wavelength','bin_name','ngals_bin','stacked_flux/mJy','stacked_noise/mJy','mean/mJy','median/mJy','stdev/mJy'])
        ...
        if os.path.exists(save_path) == True:
            if os.path.exists(file_path) == True:
                os.remove(file_path)
                ascii.write(results_table,file_path,format='csv',fast_writer=False)
            else:
                ascii.write(results_table,file_path,format='csv',fast_writer=False)
        else:
    ...
    def plot_the_stacked_images(self,side_length,save_path):
        ...
        map_library = self.sky_library
        bin_names = list(subcatalog_library.keys())
        map_names = list(map_library.keys())
        plot_matrices_all_wvs,plot_matrices_all_wvs_noise = self.get_image_of_stack(side_length,save_path)

        if os.path.exists(save_path) == True:
            for wv in map_names:
                for name in bin_names:
                    image_name = self.cat_name + '_' + str(wv) + '_' + str(name).replace(':','_')
                    #image_name = self.cat_name + '_' + str(wv) 
                    save_name = image_name + '.jpg'
                    file_path = save_path + '\_' + save_name
    ...
    def plot_stacked_flux_increasing_gals(self,save_path):
        ...
        for iwv in range(nwv):
            ...
            for name in bin_names:
                ngals_diff_lengths = {}
                length_list = list(range(10,len(subcatalog_library[name][0])+1,4))
                if len(subcatalog_library[name][0]) > 0:
